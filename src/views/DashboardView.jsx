...
export default function DashboardView({ setCurrentView, setScriptToAnalyze }) {
    ...
    fetchDashboard()
      ...
      .catch(err => {
        ...
        setData({
          ...
          optimized_count: 5,
          recent_predictions: [
            { script_title: "50 Lakh Real Estate Secret Deal", ml_score: 88.4, ann_score: 97.8, final_score: 93.1, status: "High Potential" },
            { script_title: "0% Brokerage Property Launch", ml_score: 82.0, ann_score: 91.5, final_score: 86.8, status: "High Potential" },
            { script_title: "3 Financial Hacks to Save 1 Lakh", ml_score: 79.5, ann_score: 89.0, final_score: 84.3, status: "High Potential" },
            { script_title: "AI Tools to Automate Content Creation", ml_score: 85.0, ann_score: 94.2, final_score: 89.6, status: "High Potential" }
          ]
        });
        setLoading(false);
      });
  ...
  const metrics = [
    { label: 'Total Scripts', value: data?.total_scripts || 9, icon: FileText, change: 'Active Library', color: 'text-[#4353ff]', bg: 'bg-indigo-50' },
    { label: 'Reels Evaluated', value: data?.reels_analyzed || 5, icon: Brain, change: 'Dual AI Model', color: 'text-purple-600', bg: 'bg-purple-50' },
    { label: 'Average Score', value: `${data?.avg_score || 88.5}%`, icon: Target, change: 'Benchmark: 80%', color: 'text-amber-600', bg: 'bg-amber-50' },
    { label: 'Peak Score', value: `${data?.best_score || 95.2}%`, icon: Trophy, change: 'Highest Recorded', color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { label: 'High Potential', value: data?.optimized_count || 5, icon: Sparkles, change: 'Optimized Candidates', color: 'text-blue-600', bg: 'bg-blue-50' },
  ];
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 md:space-y-8 max-w-7xl mx-auto font-sans">
      {/* Memberstack Hero Banner */}
      <div className="card-saas p-6 md:p-8 bg-gradient-to-r from-[#1a1a2e] via-[#242442] to-[#4353ff] text-white flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative overflow-hidden shadow-xl shadow-[#4353ff]/10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-5">
        {metrics.map((m, idx) => {
          const Icon = m.icon;
          return (
            <div key={idx} className="card-saas p-4 md:p-5 flex flex-col justify-between">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-wider text-slate-400">{m.label}</span>
                <div className={`w-7 h-7 md:w-8 md:h-8 rounded-xl ${m.bg} ${m.color} flex items-center justify-center`}>
                  <Icon className="w-3.5 h-3.5 md:w-4 md:h-4" />
                </div>
              </div>
              <div>
                <div className="text-xl md:text-2xl font-extrabold text-[#1a1a2e] tracking-tight">{m.value}</div>
                <div className="text-[10px] md:text-[11px] font-semibold text-slate-500 mt-1 flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Table Column */}
        <div className="lg:col-span-2 card-saas p-5 md:p-6 space-y-4">
        ...
        <div className="card-saas p-5 md:p-6 space-y-5 flex flex-col justify-between">
          <div>
